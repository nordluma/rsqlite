...
///
/// This function interprets a sequence of bytes starting from the given `offset` in the `buffer`,
/// decoding a varint using a format where each byte contributes 7 bits to the result. Bytes with
/// the most significant bit set (`1xxx_xxxx`) indicate that additional bytes follow, while the
/// last byte in the sequence most significant bit unset (`0xxx_xxxx`).
///
/// # Returns
///
/// A tuple `(u8, i64)` where:
/// * The `u8` value represents the number of bytes read from the buffer (i.e. the size of the varint)
/// * The `i64` value is the decoded integer result.
///
/// # Panics
///
/// This function will panic if `offset` is out of bounds for the `buffer`.
///
/// # Note
///
